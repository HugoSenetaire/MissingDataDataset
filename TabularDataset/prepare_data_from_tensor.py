...
def get_dataset_from_tensor(
    args_dict,
):
    data_true, Y, parameters = tensor_dataset_loader(
        args_dict["dataset_name"], args=args_dict
    )
    ...
    data_train, data_val, target_train, mask_val = train_test_split(
        data_true, Y, test_size=0.1, random_state=args_dict["seed"]
    )
    data_train, data_test, target_train, mask_test = train_test_split(

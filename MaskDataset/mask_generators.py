''' This code is based on the following repository: https://github.com/tigvarts/vaeac/blob/master/mask_generators.py'''
import numpy as np
import torch
...
class RandomPattern:
    ...
    def __call__(self, batch, density_std=0.05):
        ...
        """
        batch_size, num_channels, width, height = batch.shape
        res = torch.zeros_like(batch, device=batch.device)
...
class MixtureMaskGenerator:
    ...
    def __call__(self, batch):
        ...
        for i, gen in enumerate(self.generators):
            ...
            if len(ids) == 0:
                continue
            samples = gen(batch[ids]).to(batch.device)
            mask[ids] = samples
        return mask

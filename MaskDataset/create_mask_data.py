...
def create_mask_dataset(
    ...
    for attr_name in ["dataset_train", "dataset_test", "dataset_val"]:
        ...
        try:
            ...
            X = torch.stack([dataset.__getitem__(i)[0] for i in range(len(dataset))])
            Y = torch.stack([dataset.__getitem__(i)[1] for i in range(len(dataset))])
        mask_X, mask_Y, patterns = create_mask(
            X, Y, args=args_dict, seed=args_dict["seed"]
        )
        mask_X = 1 - mask_X.to(torch.float32)
        list_datasets.append(
            DatasetMaskAugmented(
                dataset,
                mask_X,
            )
        )
    ...
    complete_masked_dataset = CompleteDatasets(
        list_datasets[0],
        list_datasets[1],
        list_datasets[2],
...
def create_dataset_without_mask(
    complete_dataset,
):
    """
    ...
    new_dataset_train = torch.utils.data.TensorDataset(
        torch.stack(data),
        torch.stack(target),
    )
    new_dataset_train_mask_augmented = DatasetMaskAugmented(
        new_dataset_train,
        torch.stack(mask),
    )

    new_complete_dataset = CompleteDatasets(
        dataset_train=new_dataset_train_mask_augmented,
